      ...
      <form @submit.prevent="handleSave">
        <div class="space-y-4">
          ...
          <div class="relative">
            ...
            <input type="text" id="food" v-model="searchTerm" @input="debouncedSearch" placeholder="예: 닭가슴살 200g" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm" required>
            <ul v-if="searchResults.length > 0 && searchTerm" class="absolute z-10 w-full bg-white border border-gray-300 rounded-md shadow-lg mt-1 max-h-60 overflow-auto">
              <li v-for="foodItem in searchResults" :key="foodItem.id" @click="selectFoodItem(foodItem)" class="px-4 py-2 cursor-pointer hover:bg-gray-100">
                {{ foodItem.name }} ({{ foodItem.calories }} kcal)
              </li>
            </ul>
          </div>
          <div>
...
import { foodSearch } from '../../../api'; // Import the foodSearch API
...
watch(() => props.dietRecord, (newRecord) => {
  if (newRecord) {
    ...
    searchTerm.value = newRecord.food; // Initialize searchTerm with existing food
  } else {
    form.value = {
      id: null,
      date: new Date().toISOString().slice(0, 10),
    ...
    searchTerm.value = ''; // Clear searchTerm for new records
  }
  searchResults.value = []; // Clear search results on record change
}, { immediate: true });

// Watch searchTerm to update form.food
watch(searchTerm, (newVal) => {
  form.value.food = newVal;
});
...
const debouncedSearch = () => {
  ...
  searchTimeout = setTimeout(async () => {
    if (searchTerm.value.trim()) {
      try {
        const results = await foodSearch(searchTerm.value.trim());
...
const selectFoodItem = (foodItem) => {
  searchTerm.value = foodItem.name;
  form.value.food = foodItem.name;
  form.value.calories = foodItem.calories;
