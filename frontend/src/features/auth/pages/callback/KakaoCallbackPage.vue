<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-100">
    <div class="text-center">
      <h1 class="text-2xl font-semibold text-gray-800 mb-4">카카오 로그인 처리 중...</h1>
...
onMounted(async () => {
  ...
  sessionStorage.removeItem('kakao_state'); // Clear social login state
  ...
  try {
    ...
    const { accessToken, refreshToken, requiresAdditionalInfo, isTempPassword, userId, name } = response.data;

    if (accessToken && refreshToken) {
      authStore.setTokens(accessToken, refreshToken, true); // Assuming social logins imply "keep logged in"
      authStore.setUserInfo(userId, name, true);
    ...
    console.error('카카오 로그인 처리 실패:', error.response ? error.response.data : error.message);
